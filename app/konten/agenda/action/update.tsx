...
function Update({ reload, agenda }: { reload: Function; agenda: Agenda }) {
  const [tanggal, setTanggal] = useState(agenda.tanggal);
  const [judul, setJudul] = useState(agenda.judul);
  const [deskripsi, setDeskripsi] = useState(agenda.deskripsi);
  const [alamat, setAlamat] = useState(agenda.alamat);
  const [fotoUrlSelect, setFotoUrlSelect] = useState(
    `${apiImg}/${agenda.gambar}`
  );
  ...
  const handleSubmit = async (e: SyntheticEvent) => {
    ...
    let urlGambar;
    const newGambar = fotoUrlSelect != `${apiImg}/${agenda.gambar}`;
    if (newGambar) {
      urlGambar = await uploadGambar(
        image as File,
        "pemilu/agenda",
        String(agenda.gambar)
      );
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Modal.Body>
            ...
            <div className="mb-3 row">
              <label className="col-sm-4 col-form-label">Foto Genda</label>
            ...
            <Editor
              ...
              apiKey={tinymceKey}
              initialValue={String(agenda.deskripsi)}
              init={{
                height: 600,
